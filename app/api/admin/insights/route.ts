...
Generate insights in the required JSON format.`,
    ...
    try {
      ...
      insights = {
        salesTrends: {
          summary: `Revenue this week: KES${currentRevenue.toFixed(2)} (${revenueChange > 0 ? "+" : ""}${revenueChange.toFixed(1)}% vs last week)`,
          highlights: [
            `${revenuePeriod.currentOrderCount || 0} orders this week`,
            `Average order value: KES${avgOrderValue.toFixed(2)}`,
            topProducts[0]
